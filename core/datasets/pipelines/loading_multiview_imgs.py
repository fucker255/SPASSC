...
class LoadMultiViewImageFromFiles(object):
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        if self.load_stereo_depth:
            ...
            # stereo_depth_left_path = results['stereo_depth_left_path']
            # stereo_depth_left = np.load(stereo_depth_left_path)
            # stereo_depth_left = Image.fromarray(stereo_depth_left)
            
            
            # stereo_depth_right_path = results['stereo_depth_right_path']
            # stereo_depth_right = np.load(stereo_depth_right_path)
            # stereo_depth_right = Image.fromarray(stereo_depth_right)
            
            resize, resize_dims, crop, flip, rotate = img_augs
            stereo_depth = self.img_transform_core(stereo_depth, resize_dims=resize_dims,
                    crop=crop, flip=flip, rotate=rotate)
            # stereo_depth_left = self.img_transform_core(stereo_depth_left, resize_dims=resize_dims,
            #         crop=crop, flip=flip, rotate=rotate)
            # stereo_depth_right = self.img_transform_core(stereo_depth_right, resize_dims=resize_dims,
                    # crop=crop, flip=flip, rotate=rotate)
            results['stereo_depth'] = self.ToTensor(stereo_depth)
            # results['stereo_depth_left'] = self.ToTensor(stereo_depth_left)
            # results['stereo_depth_right'] = self.ToTensor(stereo_depth_right)
